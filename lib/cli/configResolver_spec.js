...
describe('ConfigResolver functionalities', () => {
  ...
  it('should get the default depolyed tests url', () => {
    let cg = new ConfigResolver();
  ...
  it('should tell to use the embeded server if no deployed tests url is given', () => {
    let cg = new ConfigResolver();
  ...
  it('should get the test path', () => {
    ...
    expect(cg.getTestPath()).to.equal('/test/');
  });

  it('should tell if we must watch files change', () => {
    let cg = new ConfigResolver();

    expect(cg.watchChanges()).to.be.false;
  });
  ...
  it('should update with the console argument', () => {
    ...
    expect(configResolver.getDeployedTestsUrl()).to.equal('http://toto');
    expect(configResolver.useEmbededServer()).to.be.false;
    expect(configResolver.watchChanges()).to.be.true;
  });
});
